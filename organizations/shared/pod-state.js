(function () {
  ...
  const demoUsers = Array.isArray(globalThis.SovereignDemoUsers) ? globalThis.SovereignDemoUsers : [];
  const identitySeed = demoUsers.map((user) => {
    const pod = Array.isArray(user.pods) ? user.pods[0] : null;
    return {
      webId: `did:web:demo.sovereign.ngo:person:${pod.name}`,
      displayName: user.profile.displayName,
      podName: pod.name,
      starterRecords: Array.isArray(pod.records) ? pod.records : [],
      starterCredentials: Array.isArray(pod.credentials) ? pod.credentials : [],
      isStaff: false
    };
  });
  identitySeed.push({
  ...
  const buildSeedState = () => {
    const ts = nowIso();
    const solidSession = globalThis.SovereignSolidSession;
    const podSelection = solidSession && typeof solidSession.getSelection === 'function'
      ? solidSession.getSelection()
      : null;
    const authenticatedWebId = asTrimmed(solidSession && solidSession.info.webId);
    const selectedPersona = identitySeed.find((entry) => (
      !entry.isStaff && (
        (authenticatedWebId && authenticatedWebId.includes(`/${entry.podName}/`)) ||
        (podSelection && new URL(podSelection.podBaseUrl).pathname === `/${entry.podName}/`)
      )
    ));
    const selectedWebId = authenticatedWebId || (selectedPersona && podSelection
      ? new URL('profile/card#me', podSelection.podBaseUrl).toString()
      : '');
    const activeIdentitySeed = selectedPersona
      ? [selectedPersona, identitySeed.find((entry) => entry.isStaff)]
      : identitySeed;
    const identities = activeIdentitySeed.map((entry) => ({
      webId: selectedPersona && entry === selectedPersona ? selectedWebId : entry.webId,
    ...
    const seededState = {
      ...
      identities,
      credentials: [],
      records: selectedPersona ? (selectedPersona.starterRecords || []).map((record) => {
        const provider = providerById(record.providerId);
        return {
          id: uid('rec'),
          subjectWebId: selectedWebId,
          providerId: record.providerId,
          providerDid: providerDidFromId(record.providerId),
          providerLabel: provider ? provider.label : record.providerId,
          category: record.category,
          summary: record.summary,
          details: { startupProvisioned: true },
          actorWebId: selectedWebId,
          createdAt: ts
        };
      }) : [],
      consents: [],
    ...
    if (selectedPersona) {
      selectedPersona.starterCredentials.forEach((credential) => {
        credentials.push(createCredentialEntry({
          credentialType: credential.type,
          issuerProviderId: credential.issuerProviderId,
          issuerProviderDid: credential.issuerProviderDid,
          subjectWebId: selectedWebId,
          subjectName: selectedPersona.displayName,
          issuedAt: ts,
          status: 'active',
          state: seededState,
          claims: { displayName: selectedPersona.displayName, ...(credential.claims || {}) },
          metadata: { startupProvisioned: true, synthetic: true }
        }));
      });
    }
  ...
  const resolveBackend = () => {
    const configRoot = typeof globalThis !== 'undefined' ? globalThis.SovereignPodConfig : null;
    ...
    const backendFactories = typeof globalThis !== 'undefined' ? globalThis.SovereignPodStorage : null;
    if (!backendFactories || typeof backendFactories !== 'object') {
      throw new Error('Pod storage factories are unavailable.');
    }

    if (backendMode === 'solid-pod') {
      if (typeof backendFactories.createSolidPodBackend !== 'function') {
        throw new Error('Solid pod backend requested, but SovereignPodStorage is unavailable.');
      }
      return backendFactories.createSolidPodBackend({
        ...
        fetch: backendConfig.fetch,
        fetchInit: backendConfig.fetchInit
      });
    }

    if (typeof backendFactories.createMemoryBackend !== 'function') {
      throw new Error('In-memory pod storage factory is unavailable.');
    }

    return backendFactories.createMemoryBackend({
      channelName: CHANNEL_NAME
    });
  };
  ...
  const saveState = (state, reason) => {
    ...
    try {
      if (!backend || typeof backend.writeState !== 'function') {
        throw new Error('Pod storage is missing writeState.');
      }
  ...
  const issueRecordAccessPass = async ({
    ...
    const accessPass = {
      ...
      accessEndpoint: `/providers/${encodeURIComponent(sourceProviderId)}/presentations/submit`,
      createdAt: now,
      expiresAt,
      status: 'active',
      issuedByWebId: actorWebId || '',
